...
async def validate_student(student_id: str) -> dict:
    ...
    adviser_check = await supabase.table("advisers").select("id").eq("id_number", student_id).execute()
    admin_check = await supabase.table("admins").select("id").eq("id_number", student_id).execute()
    ...
    result = await supabase.table("students").select("id, student_id, full_name, program, year_level").eq("student_id", student_id).execute()
...
async def get_or_create_voting_pin(student_uuid: str) -> str:
    ...
    supabase = await get_async_supabase()
    res = await supabase.table("students").select("voting_pin").eq("id", student_uuid).execute()
    ...
    await supabase.table("students").update({"voting_pin": new_pin}).eq("id", student_uuid).execute()
    return new_pin
...
async def cast_votes(student_id_str: str, election_id: str, votes: list, voting_pin: str) -> dict:
    """Insert vote records via RPC and mark the student as voted. Returns receipt info."""
    supabase = await get_async_supabase()
    student_result = await supabase.table("students").select("id, voting_pin").eq("student_id", student_id_str).execute()
    ...
    if not stored_pin or voting_pin != stored_pin:
        raise HTTPException(status_code=403, detail="Invalid Voting PIN. Please enter the correct PIN to cast your vote.")

    # Use the High-Concurrency RPC for atomic updates
    try:
        rpc_result = await supabase.rpc("cast_ballot_v2", {
        ...
        if isinstance(data, list) and len(data) > 0:
            ...
            await supabase.table("students").update({"voting_pin": None}).eq("id", student_uuid).execute()
            return data[0]
        ...
        raise HTTPException(status_code=400, detail=f"Database error during voting: {error_msg}")


async def get_vote_summary(student_id: str, election_id: str) -> dict:
    """Get the vote summary for a student in a specific election."""
    supabase = await get_async_supabase()
    student_result = await supabase.table("students").select("id, student_id, full_name").eq("student_id", student_id).execute()
    if not student_result.data:
