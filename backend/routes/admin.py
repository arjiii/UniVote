...
from models import ElectionCreate, ElectionStatusUpdate, StudentManualCreate, StudentUpdate, AdviserCreate
...
@router.get("/students")
async def get_students(user: AuthUser = Depends(require_admin)):
    """Retrieve all students (voter list)."""
    supabase = await get_async_supabase()
    res = await supabase.table("students").select("id, student_id, full_name, program, year_level, has_voted").order("full_name").execute()
...
# Adviser Management
@router.get("/advisers")
async def get_advisers(user: AuthUser = Depends(require_admin)):
    supabase = await get_async_supabase()
    res = await supabase.table("advisers").select("id, full_name, email, department, created_at").execute()
    return {"data": res.data}
...
async def create_adviser(
    ...
    # Check if exists
    supabase = await get_async_supabase()
    exists = await supabase.table("advisers").select("id").eq("email", adviser.email).execute()
    if exists.data:
        raise HTTPException(status_code=400, detail="Adviser with this email already exists.")
    
    hashed = argon2.hash(adviser.password)
    res = await supabase.table("advisers").insert({
        "email": adviser.email,
...
async def get_audit_log(user: AuthUser = Depends(require_admin)):
    return {"data": await audit_service.get_audit_log()}
